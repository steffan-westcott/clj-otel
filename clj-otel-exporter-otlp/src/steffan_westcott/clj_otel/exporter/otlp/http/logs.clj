(ns steffan-westcott.clj-otel.exporter.otlp.http.logs
  "Log record data exporter using OpenTelemetry Protocol via HTTP."
...
(defn- add-headers
  ^OtlpHttpLogRecordExporterBuilder [builder headers]
  (reduce-kv #(.addHeader ^OtlpHttpLogRecordExporterBuilder %1 %2 %3) builder headers))

(defn log-record-exporter
  "Returns a log record exporter that sends log records using OTLP via HTTP, using
  ...
  (^OtlpHttpLogRecordExporter
   ...
   (let [builder (cond-> (OtlpHttpLogRecordExporter/builder)
