...
`clj-otel` is tested with Clojure 1.12.2 and is based on the reference https://github.com/open-telemetry/opentelemetry-java[OpenTelemetry for Java] implementation, which supports Java 8 and higher.
...
There is a callback-based API and support for `CompletableFuture`.
The examples include applications using `CompletableFuture` with https://github.com/mpenet/auspex[`auspex`] and `core.async` with https://github.com/xadecimal/async-style[`async-style`].
