...
There is support for `core.async` channels, `CompletableFuture`, `Manifold` deferreds and a callback API.
The examples include applications using `core.async` with https://github.com/xadecimal/async-style[`async-style`], `CompletableFuture` with https://github.com/mpenet/auspex/[`auspex`], https://github.com/funcool/promesa[`promesa`] and https://github.com/igrishaev/whew[`whew`], and https://github.com/clj-commons/manifold[`Manifold`] deferreds.
