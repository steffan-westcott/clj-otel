...
- [ADD] New macros `chan-span/async-bound-chan-span` and `chan-span/chan-span-binding`, supporting wrapping of spans around `core.async` channels, using both bound and explicit context.
`core.async` is an optional dependency; it must be added as a dependency to an application to use the new macros.
...
https://github.com/xadecimal/async-style[`async-style`] and https://github.com/mpenet/auspex/[`auspex`] libraries are used in the examples for higher level wrappers over `core.async` and `CompletableFuture`, but neither library is used by `clj-otel` itself.
- [ADD] New example `anonymise-app` that demonstrates async spans with several async libraries: `async-style`, `auspex`, https://github.com/funcool/promesa[`promesa`] and https://github.com/igrishaev/whew[`whew`].
