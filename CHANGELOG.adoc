...
- [NEW] Modules `clj-otel-adapter-log4j` and `clj-otel-adapter-logback`, bound context aware appender and context data provider for Log4j and Logback.
...
- [NEW] Module `clj-otel-instrumentation-runtime-telemetry`, replaces former modules `clj-otel-instrumentation-runtime-telemetry-java8` and `clj-otel-instrumentation-runtime-telemetry-java17`
...
- [FIX] Simplified `util/qualified-name` to only convert kebab case strings.
This ensures `attributes/->attributes` does not add unwanted `_` characters in attribute names with digits such as `"aws.s3.bucket"` and `"k8s.pod.uid"`.
- [FIX] Allow `wrap-route` to be used within any context.
Previously, for synchronous handlers it was assumed the current context contained the server span.
...
- Bump deps
* [MAINT] OpenTelemetry `1.66.0`, semconv `1.43.0`, instrumentation `2.31.1`
** Removed obsolete module `clj-otel-exporter-zipkin`
** Added option `:record-min-max` to histograms when using `clj-otel-sdk`
** Added option map to fn `steffan-westcott.clj-otel.resource.resources/process-resource`
