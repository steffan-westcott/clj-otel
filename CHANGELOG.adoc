...
- [FIX] Removed Clojure dependency from `clj-otel` modules.
`clj-otel` consumers should provide the Clojure version to use in their dependencies.
...
The microservices examples have been extended to demonstrate use of these macros.
...
- [ADD] Refactored async microservices examples to use https://github.com/xadecimal/async-style[`async-style`], a higher level library over `core.async`.
- Bump deps
