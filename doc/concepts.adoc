...
OpenTelemetry Java provides a Java API for instrumentation with trace, metrics and logs telemetry signals.
The API is used by consuming applications in different ways:

* For traces and metrics signals, the API is used by the application directly.
* For the logs signal, the API is intended for use by logging libraries only and not directly by the application.
OpenTelemetry Java provides appenders for existing Java logging libraries.

== Using the OpenTelemetry Java SDK

The OpenTelemetry Java SDK implements the creation, sampling, batching and export of telemetry data.
The SDK acts as an implementation of the OpenTelemetry Java API.
...
An OpenTelemetry distribution (or "distro") supplied by a vendor is a repackaging of reference OpenTelemetry software, customised for ease of use with the vendor's products.
