...
It is used as a default for many functions in this library `clj-otel` and the underlying Java library `opentelemetry-java`.
...
The async function must maintain a reference to the associated context during evaluation, rather than use the current context.
...
*Sampling* is the process of selecting some elements from a set and deriving observations on the complete set using analysis of those selected elements.
...
Some backends may also apply sampling to trace data they receive, either automatically or with some developer intervention.
...
Configuration of the OpenTelemetry agent uses the same mechanism.

See documentation for https://github.com/open-telemetry/opentelemetry-java/tree/main/sdk-extensions/autoconfigure[SDK autoconfiguration] and https://github.com/open-telemetry/opentelemetry-java-instrumentation/blob/main/docs/agent-config.md[agent configuration].
