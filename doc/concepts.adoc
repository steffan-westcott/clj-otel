...
By querying an observable system, *unexpected types of issue* can be detected, classified and monitored as they arise, addressing the issue of "unknown unknowns".
...
=== What role does OpenTelemetry fulfill?
...
OpenTelemetry supports various backends, including OSS projects (Jaeger, Zipkin, Prometheus, Grafana Tempo, SigNoz) and commercial products (Honeycomb, Lightstep, Aspecto, Dynatrace, Datadog and more).
...
It removes the need to operate multiple agents or collectors to work with multiple telemetry data formats and backends.
...
=== Status and adoption of OpenTelemetry
...
The following sections are incomplete selections of OSS and commercial backends that accept telemetry data from OpenTelemetry.
...
NOTE: Some commercial telemetry backends have a free version with reduced capacity or feature set.
...
For the Java platform, automatic instrumentation is performed by the *OpenTelemetry instrumentation agent*, a Java agent that runs with the application.
...
This library `clj-otel` wraps the OpenTelemetry API for Java in an idiomatic Clojure facade.

IMPORTANT: Manual instrumentation program code depends on the OpenTelemetry API, never the OpenTelemetry SDK.
The SDK is an implementation of the API and should appear only as an [.underline]#application runtime# dependency.
...
TIP: Make use of automatic instrumentation if possible for your application, as this is a quick way to get high quality telemetry for almost no effort.
...
* The span kind indicates the relationship between the span and its parents and children in the trace.
...
* The events are timestamped records that may include attributes. Exceptions thrown during a span are captured as events.
...
==== Examples of text map propagators

== Sampling of traces

* Head vs. tail based sampling of traces
