...
* Use `steffan-westcott.otel.context/\->headers` to get headers to merge (inject) in the HTTP request to be issued.
+
...
** _Optional:_ `io.grpc/grpc-netty-shaded`, `io.grpc/grpc-protobuf` and `io.grpc/grpc-stub` to use Netty for gRPC transport rather than the default OkHttp (see example below).
This is not needed if gRPC is not used by any exporters or the application.
* On application start, use `steffan-westcott.otel.sdk.otel-sdk/init-otel-sdk!` to configure and set an OpenTelemetry SDK instance as the global OpenTelemetry instance.
* On application end, use `steffan-westcott.otel.sdk.otel-sdk/close-otel-sdk!` to close down activities of the SDK instance.
