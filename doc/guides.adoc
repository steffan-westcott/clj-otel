...
Choose the relevant option based on the use of current, bound or explicit context:
...
Choose the relevant option based on the type of asynchronous code and use of bound or explicit context:

===== Create an asynchronous span around a `core.async` channel

NOTE: `clj-otel` supports creating spans around `core.async` channels which are settled with a single result or exception value.
Channels used in this manner can be considered to work like promises.

* Ensure `org.clojure/core.async` has been added as a dependency to your application.

TIP: Consider using a higher level library for working with promise-like `core.async` channels, such as https://github.com/xadecimal/async-style[`async-style`].

====== With bound context

* Use https://cljdoc.org/d/com.github.steffan-westcott/clj-otel-api/CURRENT/api/steffan-westcott.clj-otel.api.trace.chan-span#async-bound-chan-span[`steffan-westcott.clj-otel.api.trace.chan-span/async-bound-chan-span`] to wrap a channel in a new span, where the context to use is the bound context:
+
[source,clojure]
----
(defn <get-nums [args]
  (chan-span/async-bound-chan-span {:name      "Getting numbers"
                                    :span-kind :client}
    (<fetch-nums args))) ; channel
----

====== With explicit context

* Use https://cljdoc.org/d/com.github.steffan-westcott/clj-otel-api/CURRENT/api/steffan-westcott.clj-otel.api.trace.chan-span#chan-span-binding[`steffan-westcott.clj-otel.api.trace.chan-span/chan-span-binding`] to wrap a channel in a new span, where the context to use is passed in as the `:parent` option.
The new context containing the new span is bound to `context*` in this example:
+
[source,clojure]
----
(defn <get-nums [context args]
  (chan-span/chan-span-binding [context* {:parent    context
                                          :name      "Getting numbers"
                                          :span-kind :client}]
    (<fetch-nums context* args))) ; channel
----

===== Create an asynchronous span around `CompletableFuture` async code

TIP: There are several libraries available that wrap Java's `CompletableFuture` for more idiomatic use in Clojure, such as https://github.com/mpenet/auspex/[`auspex`], https://github.com/funcool/promesa[`promesa`] and https://github.com/igrishaev/whew[`whew`].

====== With bound context

* Use https://cljdoc.org/d/com.github.steffan-westcott/clj-otel-api/CURRENT/api/steffan-westcott.clj-otel.api.trace.span#async-bound-cf-span[`steffan-westcott.clj-otel.api.trace.span/async-bound-cf-span`] to wrap a `CompletableFuture` in a new span, where the context to use is the bound context:
+
[source,clojure]
----
(defn <get-nums [args]
  (span/async-bound-cf-span {:name      "Getting numbers"
                             :span-kind :client}
    (<fetch-nums args))) ; CompletableFuture
----

====== With explicit context

* Use https://cljdoc.org/d/com.github.steffan-westcott/clj-otel-api/CURRENT/api/steffan-westcott.clj-otel.api.trace.span#cf-span-binding[`steffan-westcott.clj-otel.api.trace.span/cf-span-binding`] to wrap a `CompletableFuture` in a new span, where the context to use is passed in as the `:parent` option.
The new context containing the new span is bound to `context*` in this example:
+
[source,clojure]
----
(defn <get-nums [context args]
  (span/cf-span-binding [context* {:parent    context
                                   :name      "Getting numbers"
                                   :span-kind :client}]
    (<fetch-nums context* args))) ; CompletableFuture
----

===== Create an asynchronous span around a `Manifold` deferred

* Ensure `manifold/manifold` has been added as a dependency to your application.

====== With bound context

* Use https://cljdoc.org/d/com.github.steffan-westcott/clj-otel-api/CURRENT/api/steffan-westcott.clj-otel.api.trace.d-span#async-bound-d-span[`steffan-westcott.clj-otel.api.trace.d-span/async-bound-d-span`] to wrap a deferred in a new span, where the context to use is the bound context:
+
[source,clojure]
----
(defn <get-nums [args]
  (d-span/async-bound-d-span {:name      "Getting numbers"
                              :span-kind :client}
    (<fetch-nums args))) ; deferred
----

====== With explicit context

* Use https://cljdoc.org/d/com.github.steffan-westcott/clj-otel-api/CURRENT/api/steffan-westcott.clj-otel.api.trace.d-span#d-span-binding[`steffan-westcott.clj-otel.api.trace.d-span/d-span-binding`] to wrap a deferred in a new span, where the context to use is passed in as the `:parent` option.
The new context containing the new span is bound to `context*` in this example:
+
[source,clojure]
----
(defn <get-nums [context args]
  (d-span/d-span-binding [context* {:parent    context
                                    :name      "Getting numbers"
                                    :span-kind :client}]
    (<fetch-nums context* args))) ; deferred
----

===== Create an asynchronous span around a `Missionary` task

* Ensure `missionary/missionary` has been added as a dependency to your application.

WARNING: Missionary does not support dynamic var bindings in tasks or flows.
Therefore, it is not possible to use bound context.

====== With explicit context

* Use https://cljdoc.org/d/com.github.steffan-westcott/clj-otel-api/CURRENT/api/steffan-westcott.clj-otel.api.trace.task-span#task-span-binding[`steffan-westcott.clj-otel.api.trace.task-span/task-span-binding`] to wrap a task in a new span, where the context to use is passed in as the `:parent` option.
The new context containing the new span is bound to `context*` in this example:
+
[source,clojure]
----
(defn <get-nums [context args]
  (task-span/task-span-binding [context* {:parent    context
                                          :name      "Getting numbers"
                                          :span-kind :client}]
    (<fetch-nums context* args))) ; task
----

===== Create an asynchronous span around code with callbacks

====== With bound context

* Use https://cljdoc.org/d/com.github.steffan-westcott/clj-otel-api/CURRENT/api/steffan-westcott.clj-otel.api.trace.span#async-bound-span[`steffan-westcott.clj-otel.api.trace.span/async-bound-span`] to start a new span, where the context to use is the bound context.
The new span ends when either success/failure callback `respond`/`raise` is evaluated:
+
[source,clojure]
----
...
* Use https://cljdoc.org/d/com.github.steffan-westcott/clj-otel-api/CURRENT/api/steffan-westcott.clj-otel.api.trace.span#async-span[`steffan-westcott.clj-otel.api.trace.span/async-span`] to start a new span, where the context to use is passed in as the `:parent` option.
The new spans ends when either success/failure callback `respond`/`raise` is evaluated:
+
[source,clojure]
----
...
----
{;; ...
 :deps {com.github.steffan-westcott/clj-otel-instrumentation-runtime-telemetry-java8 {:mvn/version "0.2.10"}}}
----
...
----
{;; ...
 :deps {com.github.steffan-westcott/clj-otel-instrumentation-runtime-telemetry-java17 {:mvn/version "0.2.10"}}}
----
...
=== Add manual logs instrumentation

NOTE: OpenTelemetry Java does not provide a logging API for use by applications.
Instead, applications should use an existing Java logging library that emits an OpenTelemetry log signal, usually provided via an appender.
For Clojure applications, existing options include Log4J, Logback and Timbre.

==== Add Log4J appender and context data

NOTE: OpenTelemetry Java provides Log4J support, but it is not bound context aware.
Follow the guide below to add a bound context aware Log4J appender and context data to an application.
...
----
{;; ...
 :deps {com.github.steffan-westcott/clj-otel-adapter-log4j {:mvn/version "0.2.11"}}}
----

* If you are using the OpenTelemetry instrumentation agent, disable its built-in Log4J support to prevent conflicts :
+
.`deps.edn`
[source,clojure]
----
...
* Configure the appender, https://github.com/open-telemetry/opentelemetry-java-instrumentation/tree/main/instrumentation/log4j/log4j-appender-2.17/library#settings-for-the-log4j-appender[configuration options] match those for the OpenTelemetry Java Log4J appender :
+
.`log4j2.properties`
[source,properties]
----
appender.otel.name = OTEL
appender.otel.type = CljOtel
appender.otel.captureMapMessageAttributes = true
# ... more options

rootLogger.appenderRef.otel.ref = OTEL
----

* In the application startup code, use `steffan-westcott.clj-otel.adapter.log4j/initialize` after the SDK has been initialised, before any log statements:

** For an application run with the OpenTelemetry instrumentation agent
+
[source,clojure]
----
(ns example.app
...
(defn init-otel []
  (log4j/initialize) ; SDK has already been initialized by agent
  (log/info "OpenTelemetry initialized"))
----

** For an application run with the autoconfigure SDK extension
+
[source,clojure]
----
(ns example.app
...
** For an application run with a programmatically configured SDK
+
[source,clojure]
----
(ns example.app
...
==== Add Logback appender and MDC

NOTE: OpenTelemetry Java provides Logback support, but it is not bound context aware.
Follow the guide below to add a bound context aware Logback appender and MDC to an application.
...
----
{;; ...
 :deps {com.github.steffan-westcott/clj-otel-adapter-logback {:mvn/version "0.2.11"}}}
----

* If you are using the OpenTelemetry instrumentation agent, disable its built-in Logback support to prevent conflicts :
+
.`deps.edn`
[source,clojure]
----
...
* Configure the appender, https://github.com/open-telemetry/opentelemetry-java-instrumentation/tree/main/instrumentation/logback/logback-appender-1.0/library#settings-for-the-logback-appender[configuration options] match those for the OpenTelemetry Java Logback appender :
+
.`logback.xml`
[source,xml]
----
...
* In the application startup code, use `steffan-westcott.clj-otel.adapter.logback/initialize` after the SDK has been initialised, before any log statements:
...
----
(ns example.app
  (:require [steffan-westcott.clj-otel.adapter.logback :as logback]
            ;; ...
             ))

(defn init-otel []
  (logback/initialize) ; SDK has already been initialized by agent
  (log/info "OpenTelemetry initialized"))
----
...
----
(ns example.service
  (:require [example.interceptors :as example-interceptors]
  ...
  (-> (conn/default-connector-map 8080)
      (conn/with-interceptors
        (concat
          ...
          [(example-interceptors/coerce-response-interceptor)]
          [interceptors/not-found]
          [route/query-params]
          [(body-params/body-params)]
          [(example-interceptors/exception-response-interceptor)]))
      (conn/with-routes #{ ... })
...
** See https://opentelemetry.io/docs/languages/java/configuration/#zero-code-sdk-autoconfigure[SDK autoconfigure configuration documentation] for details on all autoconfiguration options.
...
----
{;; ...
 :deps {com.github.steffan-westcott/clj-otel-sdk-extension-autoconfigure {:mvn/version "0.2.10"}}
 :aliases {
   :otel {:jvm-opts ["-Dotel.service.name=my-app"
                     "-Dotel.traces.exporter=otlp"
                     "-Dotel.metrics.exporter=none"
                     "-Dotel.logs.exporter=none"
                     "-Dotel.exporter.otlp.traces.protocol=grpc"]
          :extra-deps {io.opentelemetry/opentelemetry-exporter-otlp               {:mvn/version "1.54.1"}
                       io.opentelemetry.instrumentation/opentelemetry-resources   {:mvn/version "2.20.1-alpha"}
                       io.grpc/grpc-netty-shaded                                  {:mvn/version "1.76.0"}
...
----
{;; ...
 :deps {com.github.steffan-westcott/clj-otel-exporter-otlp             {:mvn/version "0.2.10"}
        com.github.steffan-westcott/clj-otel-instrumentation-resources {:mvn/version "0.2.10"}
