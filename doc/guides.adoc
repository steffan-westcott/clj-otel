...
* See https://opentelemetry.io/docs/reference/specification/metrics/supplementary-guidelines/#instrument-selection[OpenTelemetry guide] to select the appropriate instrument type to use.
...
----
* Use `steffan-westcott.clj-otel.api.metrics.instrument/add!` to add a measurement to a counter or up-down counter.
...
(defn get-red-foo [context args]
  ...
  (red-foo args))
----
* Use `steffan-westcott.clj-otel.api.metrics.instrument/record!` to record a measurement in a histogram.
The measurement may have attributes and context.
+
[source,clojure]
----
...
----
* Use https://cljdoc.org/d/com.github.steffan-westcott/clj-otel-api/CURRENT/api/steffan-westcott.clj-otel.instrumentation.runtime-metrics#register![`steffan-westcott.clj-otel.instrumentation.runtime-metrics/register!`] to add JVM runtime metrics.
