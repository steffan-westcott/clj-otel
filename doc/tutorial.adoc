...
You will first add automatic instrumentation to quickly get telemetry.
You then add manual instrumentation to enrich the telemetry data and gain further insight on the application behaviour.

This tutorial assumes a Unix-like OS equipped with Docker and a Clojure development environment.
...
You will now add more detail by adding manual instrumentation on top of the automatic instrumentation.
