...
For example, in `examples/auto-sdk-config`, to export trace data to the Collector using gRPC with Netty transport, start a REPL with aliases `otel`, `traces-collector-grpc`, `metrics-none`, `logs-none` and `grpc-netty`.
